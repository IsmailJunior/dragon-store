...
export const EditBlockPageAdmin = () =>
{
	...
	const block = landing?.blocks ? landing?.blocks[ params.block ][params.side] : null
	console.log(block)
	const onInvertHeadlineColorClickedHandler = () => {
		dispatch( updateLandingInvertTextColor( { group: 'blocks', item: params.block,side: params.side} ) );
	}
	...
	{
		dispatch( updateLanding( { group: 'blocks', item: params.block, side: params.side ,id: id } ) );
			...
			{ showItems ?
				...
				: <>
					<div className='py-16 flex flex-col items-center gap-10 p-5 shadow-md bg-white'>
						<h1 className='text-4xl font-semibold'>{ block?.name }</h1>
						...
						<FeatureBlockAdmin invert={ block?.invertText } title={ block?.name } description={ block?.description } image={ block?.block } />
					</div>
	</> }
	</div>
  )
