...
<div className="flex flex-col gap-8 items-center lg:container mt-7 mb-7">
	<h1 className='text-lg md:text-3xl font-semibold'>قائمة المنتجات التي اخترتها:</h1>
		...
		<div className='flex flex-col gap-5'>
					<h1 className='text-3xl font-semibold'>السعر: د.ع { item.price + ',000'}</h1>
					<button onClick={() => onDeleteItemClickedHandler(item.id)} className="w-80 hover:bg-blue-600 border active:bg-blue-500 border-slate-300 p-2 rounded-md bg-blue-500 text-white">ازالة من السلة</button>
		</div>
	</div>
	))}
	<hr className='w-full' />
	<div className='w-1/2 flex flex-row justify-between'>
			<h1 className='text-3xl font-semibold'>الكلي: د.ع {total + ',000'}</h1>
	</div>
</div>
)
}